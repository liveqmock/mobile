...
(function($) {
	 ...
	 * @requires
	 */
	$.widget("youi.organize", $.ui.mouse,$.extend({},{
		options:{
			width:800,
			height:420,
			distance: 1,
		 ...
		 */
		_create:function(){
			this.element.addClass('youi-organize').css({
		...
		btn_formDesign:function(refNodeId){
			...
			if(formKey){
				...
				$.youi.pageUtils.goPage('/page/organize.flow/formDesigner.html',record);
			}
		},
		...
		_propertiesSupport:function(event,data){
			if(data&&data.type){
				var selectorPath = ['.youi-organize#'+this.element.attr('id')].concat(data.selectorPath),
				...
				function propertyChange(selectorPath){
					return function(property,value){
						...
						//调用属性修改的函数
						$(selectorPath[0]).organize('setProperty',selectorPath.slice(1),property,value,event);
					};
				}
		...
		addTransition:function(fromId,toId){
			return this._addTransition(fromId,toId,false);
		},
