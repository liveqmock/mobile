...
public class ${modelClassName}ManagerImpl extends <#if supportWorkflow==true>Workflow</#if>BaseManagerImpl<#if supportWorkflow==true><${modelClassName}></#if> implements ${modelClassName}Manager{
	@Autowired
	private ${daoClassName}<${modelClassName},String> ${daoName};
    ...
    <#if hasSet??>
    public ${modelClassName} getInitialize${modelClassName}(${idType} id,String[] cAttributes) throws BusException {
...
//    	}
	 ...
	 * 获得工作流相关实体
	 */
	public ${modelClassName} getWorkflowDomain(String workflowId) throws BusException {
		return ${daoName}.getObjectByUniqueProperty("workflowId", workflowId);
	}
	/**
	 * 保存工作流相关实体
	 */
	protected ${modelClassName} saveWorkflowDomain(${modelClassName} ${modelName}) throws BusException {
